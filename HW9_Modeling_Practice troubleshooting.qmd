...
MLR_final <- MLR_wkf |>
  ...
  compute_metrics(MLR_final, metric_set(rmse, mae))
```
...
LASSO_final <- LASSO_wkf |>
  ...
  compute_metrics(LASSO_final, metric_set(rmse, mae))
```
...
RTREE_final <- RTREE_wkf |>
  ...
  compute_metrics(RTREE_final, metric_set(rmse, mae))
```
...
```{r}
# Test without tree_depth parameter first
BTREE_model <- bag_tree(min_n = 10, cost_complexity = tune()) |>
set_engine("rpart") |>
set_mode("regression")
```
...
### Debug: Try without recipe to isolate the problem
```{r}
# Test 1: Try without any recipe at all
BTREE_no_recipe <- bag_tree(cost_complexity = 0.01) |>
  set_engine("rpart") |>
  set_mode("regression")

# Create minimal dataset for testing
bike_minimal <- bike_train_data |> 
  select(SUM_RBC, `TEMP(deg C)_MEAN`, `RH(%)_MEAN`, `WD_SPD(m/s)_MEAN`) |>
  drop_na()

BTREE_minimal_fit <- BTREE_no_recipe |>
  fit(SUM_RBC ~ ., data = bike_minimal)

print("Bagged tree works without recipe")
```

### Debug: Check package versions
```{r}
cat("Package versions:\n")
cat("baguette:", as.character(packageVersion("baguette")), "\n")
cat("tidymodels:", as.character(packageVersion("tidymodels")), "\n")
cat("parsnip:", as.character(packageVersion("parsnip")), "\n")
```

### Alternative: Try using different engine or simpler approach
```{r}
# If the above works, try tuning with fewer levels
...
### Pull Out the Best BTREE Model with select_best()  
```{r}
...
BTREE_final <- BTREE_wkf |>
  ...
  compute_metrics(BTREE_final, metric_set(rmse, mae))
```
