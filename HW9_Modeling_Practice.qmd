...
```
Here we can see that the MLR Model and RF Model are the best models.  they have the lowest RMSE and MAE. The question is, which one of these is the "better" model. The MLR Model has a higher RMSE than the RF Model, but the MLR Model has a lower MAE compared to the RF Model. For me, I would select the RF Model because of the lower RMSE because RMSE is in the same units as the data, which makes it easier for me to interpret what is going on.
...
∗ For the random forest model, this is a bit complicated. Check this out and see if you can get
it to work.
```{r}
final_model |> 
  extract_fit_parsnip() |> 
